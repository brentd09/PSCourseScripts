...
function Show-PipelineByVal {
  ...
  Clear-Host
  Write-Host
  Write-Host -ForegroundColor Cyan "Pipeline ByValue"
...
function Show-PipelineByPN {
  ...
  $CommandGapLength = $KeyGap - $Cmdlet1Length + 5
  $CommandGap = ' ' * $CommandGapLength
  $GuideGap   = ' ' * ($CommandGapLength + 10)
  $MappingGap = ('-' * ($CommandGapLength - $Cmdlet1Length - 12 )) + '>'
  $Cmdlet1 = $TestResultByPN.Cmdlet1 | Select-Object -Unique
  $Cmdlet2 = $TestResultByPN.Cmdlet2 | Select-Object -Unique
  Clear-Host
  Write-Host
  Write-Host -ForegroundColor Cyan "Pipeline ByPropertyName"
  Write-Host -ForegroundColor Cyan "-----------------------"
  Write-Host -ForegroundColor Yellow -NoNewline "$Cmdlet1 " 
  Write-Host -NoNewline "$CommandGap"
  Write-Host -ForegroundColor Green " $Cmdlet2"
  write-host "  | $GuideGap ^"
  write-host "  | $GuideGap |"
  write-host "  V $GuideGap |"
  foreach ($PropToParamMatch in $TestResultByPN) {
    if ($TestResultByPN.ByPNPipe -eq $true -and $PropToParamMatch.PropMatch.ShowPropType) {
      $MapGuide = ('-' * ($CommandGapLength - $($TestResultByPN.ParamMatch.ShowParamType.length))) + '>'
      Write-Host -NoNewline '  Property: '
      Write-Host -ForegroundColor Yellow -NoNewline "$($PropToParamMatch.PropMatch.ShowPropType) "
      Write-Host -NoNewline "$MapGuide"
      Write-Host -ForegroundColor Green " $($TestResultByPN.ParamMatch.ShowParamType)"
    }
  }
  Write-Debug -Message 'non props'
  foreach ($NonMatchingPropery in ($TestResultByPN.AllProperties | Where-Object {$_.Name -notin $TestResultByPN.PropMatch.Name})) {
    ...
    $NonMapGuide = ('-' * ($CommandGapLength - $($NonMatchingPropery.ShowPropType.length) - 3)) + '>X'
    #('-' * ($KeyGap - $($NonMatchingPropery.ShowPropType.length))) + '>X'
